      ...
      {pages.map((page,index) =>{
        return(
          <SwiperSlide key={index}>
            <div className='grid grid-cols-2 gap-x-5 md:grid-cols-3 lg:grid-cols-4 lg:gap-[30pz]'>
              {page.productList.map((product,index) =>{
                const{image,name,price, oldPrice}=product
                return (
                  <div className='w-full max-w-[290px] h-[380px] text-left' key={index}>
                    <div className='border hover:border-x-amber-900 rounded-[18px] w-full max-w-[250px]
                    h-full max-h-[292px] flex items-center justify-center mb-[15px] relative transition'
                    >
